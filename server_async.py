...
cloudopen_api = options.cloudopen_api_url
...
class CloudOpenAPI(object):
    ...
    def create_vm(self, vm):
        cloudopen_api_create = cloudopen_api + "/api/instance/add/"
        user_id = vm['user_id']
    ...
    def delete_vm(self, instance_id):
        cloudopen_api_delete = cloudopen_api + "/api/instance/delete/"
        body = dict()
    ...
    def query_vm(self, instance_id):
        cloudopen_api_query = cloudopen_api + "/api/instance/"
        body = dict()
        body.setdefault('id', instance_id)
...
class ProcessRequest(object):
    def process_request(self, user_id, instance_data):
        '''
        处理用户的请求
...
class MainHandler(web.RequestHandler, CloudOpenAPI, ProcessRequest):
    def get(self):
        '''
        根据用户id返回用户排队的序号
        找不到则返回-1
    ...
    def post(self):
        ...
        if check_max_instance_for_user(user_id) <= 2:
            self.process_request(user_id, instance_data)
        else:
            self.write({"status": -1})
            self.finish()
